...
int64_t _placeholder_() {
    printf("_placeholder_ function invoked, which means you have encountered code that is not yet implemented.\n");
    printf("To avoid crashing the system, the file system will exit immediately.\n");
    exit(-1);
}
...
// "Borrowed" from https://elixir.bootlin.com/linux/latest/source/fs/fat/namei_msdos.c#L19
...
int to_shortname(const char* name, size_t len, char* res) {
    ...
    // 0xe5 indicates deletion. If the first letter is `0xe5`, it must be converted to `0x05`
...
void time_fat_to_unix(struct timespec* ts, uint16_t date, uint16_t time, uint16_t acc_time) {
    struct tm t;
    memset(&t, 0, sizeof(t));
    t.tm_year = (date >> 9) + 80;            // 7-bit year
    t.tm_mon  = ((date >> 5) & 0xf) - 1;     // 4-bit month
    t.tm_mday = date & 0x1f;                 // 5-bit day

    t.tm_hour = time >> 11;                  // 5-bit hour
    t.tm_min  = (time >> 5) & 0x3f;          // 6-bit minute
    t.tm_sec  = (time & 0x1f) * 2;           // 5-bit second (precision 2 seconds)
